...
const { canModifyQueue } = require(`../modules/djCheck`);
const yt = require(`ytdl-core`);

module.exports = {
    async play(song, message) {
        let PRUNING = false;
        let SOUNDCLOUD_CLIENT_ID = require(`../../config/config`).apiKeys[1];
...
async function spotifyPlayer(song, message) {
    if (song == undefined) return;
    const queue = message.client.queue.get(message.guild.id);

    if (!queue) {
        try {
            let dispatcher = queue.connection.play(yt(song.url, {
                audioonly: true
            }))
